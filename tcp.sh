...
#	Version: 1.3.2.95
#	Author: 中国站长
#	更新内容及反馈:  https://cnwebmasters.com
#=================================================
...
#安装BBR内核
installbbr() {
  kernel_version="5.14.10"
  kernel_version_arm64="5.14.9"
  bit=$(uname -m)
  rm -rf bbr
  ...
  if [[ "${release}" == "centos" ]]; then
    ...
    if [[ ${bit} == "x86_64" ]]; then
      ...
      headurl=https://git.huaweicdn.net/packages/linux-headers-${kernel_version}_${kernel_version}-1_amd64.deb
      imgurl=https://git.huaweicdn.net/packages/linux-image-${kernel_version}_${kernel_version}-1_amd64.deb
      echo -e "正在检查headers下载连接...."
      checkurl $headurl
      echo -e "正在检查内核下载连接...."
      checkurl $imgurl
      ...
      #headurl=$(curl -s 'https://api.github.com/repos/ylx2016/kernel/releases' | grep ${github_tag} | grep 'deb' | grep 'headers' | awk -F '"' '{print $4}')
      #imgurl=$(curl -s 'https://api.github.com/repos/ylx2016/kernel/releases' | grep ${github_tag} | grep 'deb' | grep -v 'headers' | grep -v 'devel' | awk -F '"' '{print $4}')
      headurl=https://git.huaweicdn.net/packages/linux-headers-${kernel_version_arm64}_${kernel_version_arm64}-1_arm64.deb
      imgurl=https://git.huaweicdn.net/packages/linux-image-${kernel_version_arm64}_${kernel_version_arm64}-1_arm64.deb
      echo -e "正在检查headers下载连接...."
      checkurl $headurl
      echo -e "正在检查内核下载连接...."
      checkurl $imgurl
  ...
  if [[ "${release}" == "centos" ]]; then
    if [[ ${version} == "7" ]]; then
      if [[ ${bit} == "x86_64" ]]; then
        ...
        detele_kernel_head
        headurl=https://git.huaweicdn.net/packages/kernel-headers-4.14.129-bbrplus.rpm
        imgurl=https://git.huaweicdn.net/packages/kernel-4.14.129-bbrplus.rpm
        echo -e "正在检查headers下载连接...."
        checkurl $headurl
        echo -e "正在检查内核下载连接...."
        checkurl $imgurl
    ...
    if [[ ${bit} == "x86_64" ]]; then
      ...
      detele_kernel_head
      headurl=https://git.huaweicdn.net/packages/linux-headers-4.14.129-bbrplus.deb
      imgurl=https://git.huaweicdn.net/packages/linux-image-4.14.129-bbrplus.deb
      echo -e "正在检查headers下载连接...."
      checkurl $headurl
      echo -e "正在检查内核下载连接...."
      checkurl $imgurl
  ...
  if [[ "${release}" == "centos" ]]; then
    if [[ ${version} == "7" ]]; then
      if [[ ${bit} == "x86_64" ]]; then
        ...
        #headurl=$(curl -s 'https://api.github.com/repos/ylx2016/kernel/releases' | grep ${github_tag} | grep 'rpm' | grep 'headers' | awk -F '"' '{print $4}')
        #imgurl=$(curl -s 'https://api.github.com/repos/ylx2016/kernel/releases' | grep ${github_tag} | grep 'rpm' | grep -v 'headers' | grep -v 'devel' | awk -F '"' '{print $4}')
        headurl=https://git.huaweicdn.net/packages/kernel-headers-5.10.70_xanmod1-1.x86_64.rpm
        imgurl=https://git.huaweicdn.net/packages/kernel-5.10.70_xanmod1-1.x86_64.rpm
        echo -e "正在检查headers下载连接...."
        checkurl $headurl
        echo -e "正在检查内核下载连接...."
        checkurl $imgurl
      ...
      #headurl=$(curl -s 'https://api.github.com/repos/ylx2016/kernel/releases' | grep ${github_tag} | grep 'rpm' | grep 'headers' | awk -F '"' '{print $4}')
      #imgurl=$(curl -s 'https://api.github.com/repos/ylx2016/kernel/releases' | grep ${github_tag} | grep 'rpm' | grep -v 'headers' | grep -v 'devel' | awk -F '"' '{print $4}')
      headurl=https://git.huaweicdn.net/packages/kernel-headers-c8-5.10.70_xanmod1-1.x86_64.rpm
      imgurl=https://git.huaweicdn.net/packages/kernel-c8-5.10.70_xanmod1-1.x86_64.rpm
      echo -e "正在检查headers下载连接...."
      checkurl $headurl
      echo -e "正在检查内核下载连接...."
      checkurl $imgurl
  ...
  if [[ "${release}" == "centos" ]]; then
    if [[ ${version} == "7" ]]; then
      if [[ ${bit} == "x86_64" ]]; then
        ...
        headurl=https://git.huaweicdn.net/packages/CentOS-7_Optional_kernel-bbrplus-headers-5.10.71-1.bbrplus.el7.x86_64.rpm
        imgurl=https://git.huaweicdn.net/packages/CentOS-7_Required_kernel-bbrplus-5.10.71-1.bbrplus.el7.x86_64.rpm
        echo -e "正在检查headers下载连接...."
        checkurl $headurl
        echo -e "正在检查内核下载连接...."
        checkurl $imgurl
    ...
    if [[ ${version} == "8" ]]; then
      if [[ ${bit} == "x86_64" ]]; then
        ...
        headurl=https://git.huaweicdn.net/packages/CentOS-8_Optional_kernel-bbrplus-headers-5.10.71-1.bbrplus.el8.x86_64.rpm
        imgurl=https://git.huaweicdn.net/packages/CentOS-8_Required_kernel-bbrplus-core-5.10.71-1.bbrplus.el8.x86_64.rpm
        echo -e "正在检查headers下载连接...."
        checkurl $headurl
        echo -e "正在检查内核下载连接...."
        checkurl $imgurl
    ...
    if [[ ${bit} == "x86_64" ]]; then
      ...
      headurl=https://git.huaweicdn.net/packages/Debian-Ubuntu_Required_linux-headers-5.10.71-bbrplus_5.10.71-bbrplus-1_amd64.deb
      imgurl=https://git.huaweicdn.net/packages/Debian-Ubuntu_Required_linux-image-5.10.71-bbrplus_5.10.71-bbrplus-1_amd64.deb
      echo -e "正在检查headers下载连接...."
      checkurl $headurl
      echo -e "正在检查内核下载连接...."
      checkurl $imgurl
      ...
      headurl=https://git.huaweicdn.net/packages/Debian-Ubuntu_Required_linux-headers-5.10.71-bbrplus_5.10.71-bbrplus-1_arm64.deb
      imgurl=https://git.huaweicdn.net/packages/Debian-Ubuntu_Required_linux-image-5.10.71-bbrplus_5.10.71-bbrplus-1_arm64.deb
      echo -e "正在检查headers下载连接...."
      checkurl $headurl
      echo -e "正在检查内核下载连接...."
      checkurl $imgurl
...
start_menu() {
  clear
  echo && echo -e " TCP加速 一键安装管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix} from https://cnwebmasters.com
